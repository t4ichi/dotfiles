...
in
{
  ...
  home.file = {
    # tools/*（アプリが実行時に読む live 設定）
    ".config/nvim".source = link "${dots}/tools/nvim";
    ".config/ghostty/config".source = link "${dots}/tools/ghostty/config";
    ".config/herdr/config.toml".source = link "${dots}/tools/herdr/config.toml";
    ".config/mise/config.toml".source = link "${dots}/tools/mise/config.toml";
    ...
    # VSCode は programs.vscode（tools/vscode/vscode.nix）で宣言管理
  };
}
